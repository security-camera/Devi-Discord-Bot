...
def get_api_key(guild_id: int) -> str | None:
    ...
    with db_cursor() as cur:
        cur.execute("SELECT api_key FROM api_keys WHERE guild_id = ?", (guild_id,))
        row = cur.fetchone()
...
def set_api_key(guild_id: int, api_key: str):
    ...
    with db_cursor(commit=True) as cur:
        cur.execute(
            """INSERT INTO api_keys (guild_id, api_key) VALUES (?, ?)
...
def remove_api_key(guild_id: int):
    ...
    with db_cursor(commit=True) as cur:
        cur.execute("DELETE FROM api_keys WHERE guild_id = ?", (guild_id,))